...
# ========== УДАЛЕНИЕ ВСЕХ ПРЕДЫДУЩИХ СООБЩЕНИЙ ПРИ /start (только в личных чатах) ==========
async def delete_all_previous_messages(context, chat_id, current_msg_id):
    """Удаляет все сообщения с ID меньше current_msg_id."""
    try:
        for msg_id in range(0, current_msg_id):
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # В личных сообщениях удаляем ВСЕ предыдущие сообщения (включая предыдущие /start)
    if chat_type == "private":
        asyncio.create_task(delete_all_previous_messages(context, chat_id, current_msg_id))
...
async def view_article(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Для текстовых статей дополнительно отправляем ссылку отдельным сообщением
    if content_type == "text":
        await context.bot.send_message(
...
def run_bot():
    ...
    print("Бот запущен с полной очисткой чата и исправленными кнопками...")
