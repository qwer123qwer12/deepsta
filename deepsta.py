...
# Добавление endpoint ID для Neon
        ...
        if host.endswith(".neon.tech"):
            ...
            if "endpoint%3D" not in options and "endpoint=" not in options:
                if options:
                    ...
                    options = f"endpoint%3D{endpoint_id}"
            qs["options"] = [options]
            # Ручная сборка query, чтобы избежать двойного кодирования %
...
# Получение IPv4 адреса хоста
def resolve_host_ipv4(host: str):
    try:
