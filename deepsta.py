...
class Database:
    ...
    # --- Материалы ---
    async def add_material(self, title, description, link="", file_id="", file_name=""):
        def _add():
            conn = sqlite3.connect(self.db_path)
            c = conn.cursor()
            c.execute(
                "INSERT INTO materials (title, description, link, file_id, file_name) VALUES (?,?,?,?,?)",
                (title, description, link, file_id, file_name),
            )
            conn.commit()
            lid = c.lastrowid
            conn.close()
    ...
    async def get_all_materials(self):
        def _get():
            conn = sqlite3.connect(self.db_path)
            c = conn.cursor()
            c.execute("SELECT id, title, description, link, file_id, file_name FROM materials ORDER BY id DESC")
            rows = c.fetchall()
            conn.close()
            return rows
        return await asyncio.to_thread(_get)
    ...
    async def get_material(self, material_id):
        def _get():
            conn = sqlite3.connect(self.db_path)
            c = conn.cursor()
            c.execute("SELECT id, title, description, link, file_id, file_name FROM materials WHERE id=?", (material_id,))
            row = c.fetchone()
            conn.close()
            return row
        return await asyncio.to_thread(_get)
...
async def send_to_channel(context, text=None, document=None, caption=None, media_type=None):
    """Отправляет пост в закрытый канал. Поддерживает текст, фото, видео, документы."""
    try:
        if text:
...
async def broadcast_post(context, post_type, content=None, file_id=None, caption=None):
    """Рассылает пост (текст/медиа) всем подписчикам."""
    subscribers = await db.get_subscribers()
    if not subscribers:
        return
    for user_id in subscribers:
...
async def material_detail(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not material:
        ...
        return
    _, title, description, link, file_id, file_name = material
    text = f"📌 <b>{title}</b>\n\n{description}"
    if link:
...
async def download_material(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not material or not material[4]:
        ...
        return
    _, title, _, _, file_id, file_name = material
    try:
...
async def material_desc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Вы хотите прикрепить ссылку или загрузить файл?", reply_markup=keyboard)
...
def run_bot():
    ...
    # ConversationHandler для добавления статьи (PDF/медиа)
    ...
    # ConversationHandler для рассылки поста (принимает всё)
    ...
    print("Бот запущен с поддержкой медиа и постов...")
