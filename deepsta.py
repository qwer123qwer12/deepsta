...
async def view_article_by_id(update: Update, context: ContextTypes.DEFAULT_TYPE, article_id: int):
    ...
    # Если не попали в другие ветки, всё равно отправляем ссылку
    await context.bot.send_message(
        chat_id=update.effective_chat.id,
        text=f"🔗 Ссылка на статью:\n{link}"
    )
...
def run_bot():
    ...
    print("Бот запущен с длинными названиями, ссылками и постоянным хранилищем...")
