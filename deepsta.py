...
def add_neon_options(url: str) -> str:
    """Добавляет endpoint ID для Neon, если хост заканчивается на .neon.tech."""
    try:
        ...
        if host.endswith(".neon.tech"):
            ...
            if options:
                if "endpoint%3D" not in options and "endpoint=" not in options:
        ...
        logging.warning(f"Не удалось добавить options для Neon: {e}")
    return url

DATABASE_URL = add_neon_options(DATABASE_URL)

def resolve_host_ipv4(url: str) -> str:
    """Преобразует хост в IPv4-адрес, если это возможно."""
    try:
        ...
        if host:
            ...
            ipv4 = infos[0][4][0]
            return url.replace(host, ipv4, 1)
    except Exception as e:
        logging.warning(f"Не удалось преобразовать хост в IPv4: {e}")
    return url

DATABASE_URL = resolve_host_ipv4(DATABASE_URL)

if "sslmode" not in DATABASE_URL:
    DATABASE_URL += "?sslmode=require"
else:
    # Убедимся, что sslmode не дублируется
    pass
...
class Database:
    ...
    def _connect(self):
        """Подключение с отключенным channel binding (для Supabase/Neon)."""
        return psycopg2.connect(self.db_url, channel_binding="disable")
    ...
    async def add_material(self, title, description, link="", file_id="", file_name="", article_id=0):
        def _add():
            ...
            c = conn.cursor()
            c.execute(
                "INSERT INTO materials (title, description, link, file_id, file_name, article_id) VALUES (%s,%s,%s,%s,%s,%s) RETURNING id",
    ...
    if content_type == "text":
        ...
        if file_id:
            try:
                ...
                await context.bot.send_message(chat_id=update.effective_chat.id, text=f"🔗 Ссылка на статью:\n{link}")
            except Exception as e:
    ...
    if content_type == "text":
        ...
        if file_id:
            try:
                ...
                await context.bot.send_message(chat_id=query.message.chat_id, text=f"🔗 Ссылка на статью:\n{link}")
            except Exception as e:
                logger.error(f"Ошибка file_id: {e}")
...
async def broadcast_send(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.text:
        ...
        asyncio.create_task(send_to_channel(context, text=caption))
...
async def material_receive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message and update.message.text:
        ...
        await update.message.reply_text("Отправьте ссылку, файл или выберите статью.")
        return MATERIAL_LINK_OR_FILE
