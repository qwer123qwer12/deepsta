...
# ========== ЗАПУСК БОТА ==========
def run_bot():
    """Синхронная функция для запуска бота (в главном потоке)."""
...
def start_web_server():
    """Запускает Flask-сервер для пинга UptimeRobot."""
    port = int(os.environ.get("PORT", 10000))
    web_app.run(host="0.0.0.0", port=port, debug=False, use_reloader=False)

# ========== ТОЧКА ВХОДА ==========
if __name__ == "__main__":
    # Запускаем веб-сервер в дочернем процессе
    web_process = Process(target=start_web_server, daemon=True)
    web_process.start()
    # Бот работает в главном процессе
    run_bot()
else:
    # Если файл импортируется, ничего не делаем
    pass
