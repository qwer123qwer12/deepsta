...
BOT_USERNAME = os.environ["BOT_USERNAME"]  # например: my_articles_bot (без @)
MATERIALS_CHANNEL_ID = os.environ.get("MATERIALS_CHANNEL_ID")  # опционально
ARTICLES_DIR = "articles_pdf"

# Состояния
LANG, CONTENT_TYPE, TITLE, FILE = range(4)          # для добавления статьи (PDF/медиа)
BROADCAST_MESSAGE = 0                               # для рассылки поста
FEEDBACK_MESSAGE = 1                                # для обратной связи
EDIT_POST_TEXT = 2                                  # для редактирования сохранённых статей
# Состояния для материалов
MATERIAL_TITLE, MATERIAL_DESC, MATERIAL_LINK_OR_FILE, MATERIAL_ARTICLE_SELECT = range(3, 7)
...
class Database:
    ...
    def _init_db(self):
        ...
        """)
        # Материалы (с поддержкой прикрепления статей)
        c.execute("""
    ...
    async def get_text_posts(self):
        """Возвращает все посты (не PDF) для управления."""
        def _get():
            conn = sqlite3.connect(self.db_path)
            c = conn.cursor()
...
# ========== ОТПРАВКА В КАНАЛ ==========
async def send_to_channel(context, text=None, document=None, caption=None, media_type=None):
    try:
        if text:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Проверяем deep link (например, ?start=article_123)
    if update.message and update.message.text and " " in update.message.text:
        deep_link = update.message.text.split(" ", 1)[1]
        if deep_link.startswith("article_"):
            try:
                article_id = int(deep_link.split("_")[1])
                article = await db.get_article(article_id)
                if article:
                    # Проверяем подписку
                    if not await is_member(user_id, context):
                        await update.message.reply_text(
                            f"Для просмотра статьи вступите в группу {GROUP_ID} и повторите переход.",
                            reply_markup=InlineKeyboardMarkup([
                                [InlineKeyboardButton("Проверить участие", callback_data="check_sub")]
                            ])
                        )
                        return
                    # Отправляем статью как в view_article
                    await send_article_to_user(update.effective_chat.id, article, context)
                    return
            except Exception as e:
    ...
    if await can_access_materials(user_id, context):
        keyboard_buttons.append([InlineKeyboardButton("📂 Материалы", callback_data="materials_list")])
...
async def send_article_to_user(chat_id, article, context):
    """Отправляет статью пользователю (аналог view_article)."""
    art_id, title, file_id, filename, content_type, content, lang, date = article
    if content_type == "text":
        await context.bot.send_message(chat_id=chat_id, text=content, parse_mode=ParseMode.HTML)
        ...
        if file_id:
            ...
            await context.bot.send_message(chat_id=chat_id, text="Файл отсутствует.")
        ...
        try:
            await method(chat_id=chat_id, document=file_id, caption=title)
        except Exception as e:
...
async def choose_language(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for art in articles:
        ...
        # Кнопка с ссылкой для копирования
        share_link = f"https://t.me/{BOT_USERNAME}?start=article_{art_id}"
        keyboard_buttons.append(
            [InlineKeyboardButton(f"{type_emoji} {title} ({lang_tag})", callback_data=f"article_{art_id}"),
             InlineKeyboardButton("🔗", callback_data=f"share_{art_id}")]  # Кнопка "Поделиться"
        )
...
async def share_article(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    ...
    if not article:
        ...
        return
    share_link = f"https://t.me/{BOT_USERNAME}?start=article_{article_id}"
    await query.edit_message_text(
        f"Ссылка на статью «{article[1]}»:\n\n{share_link}\n\nНажмите на ссылку, чтобы скопировать её.",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("🔙 К списку статей", callback_data=f"lang_{context.user_data.get('lang', 'ru')}")]
        ])
    )
...
async def back_to_lang(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await can_access_materials(query.from_user.id, context):
        keyboard.append([InlineKeyboardButton("📂 Материалы", callback_data="materials_list")])
    keyboard.append([InlineKeyboardButton("✉️ Написать автору", callback_data="feedback_start")])
...
async def view_article(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await send_article_to_user(query.message.chat_id, article, context)
    ...
    keyboard = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🔗 Поделиться", callback_data=f"share_{art_id}")],
    ])
    if await can_access_materials(query.from_user.id, context):
        keyboard.inline_keyboard.append([InlineKeyboardButton("📂 Материалы", callback_data="materials_list")])
    keyboard.inline_keyboard.append([InlineKeyboardButton("✉️ Написать автору", callback_data="feedback_start")])
...
async def materials_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for mat in materials:
        ...
        keyboard.append([InlineKeyboardButton(title, callback_data=f"material_{mat_id}")])
    keyboard.append([InlineKeyboardButton("🔙 Назад", callback_data="back_to_welcome")])
    await query.edit_message_text("📂 Рекомендованные материалы:", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def material_detail(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = []
    if file_id:
        keyboard.append([InlineKeyboardButton("📥 Скачать файл", callback_data=f"download_material_{mat_id}")])
    if article_id:
        keyboard.append([InlineKeyboardButton("📄 Открыть статью", callback_data=f"article_{article_id}")])
    keyboard.append([InlineKeyboardButton("🔙 К списку материалов", callback_data="materials_list"),
...
async def broadcast_send(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.text:
        ...
        asyncio.create_task(send_to_channel(context, text=content))
    elif update.message.photo:
        file_id = update.message.photo[-1].file_id
        caption = update.message.caption or ""
        asyncio.create_task(broadcast_post(context, "photo", file_id=file_id, caption=caption))
        asyncio.create_task(send_to_channel(context, document=file_id, caption=caption, media_type="photo"))
    elif update.message.video:
        file_id = update.message.video.file_id
        caption = update.message.caption or ""
        asyncio.create_task(broadcast_post(context, "video", file_id=file_id, caption=caption))
        asyncio.create_task(send_to_channel(context, document=file_id, caption=caption, media_type="video"))
    elif update.message.document:
        file_id = update.message.document.file_id
        caption = update.message.caption or ""
        asyncio.create_task(broadcast_post(context, "document", file_id=file_id, caption=caption))
        asyncio.create_task(send_to_channel(context, document=file_id, caption=caption))
    else:
...
# --- Управление постами (редактирование сохранённых) ---
async def manage_posts(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for post in posts:
        ...
        keyboard.append([InlineKeyboardButton(f"{emoji} {title} ({lang})", callback_data=f"editpost_{post_id}")])
...
# --- Добавление статьи (PDF/медиа, без текстового) ---
...
async def admin_materials_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text("📂 Управление материалами:", reply_markup=keyboard)
...
async def material_desc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Вы хотите прикрепить ссылку, файл или статью?", reply_markup=keyboard)
...
async def material_link_or_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if choice == "link":
        ...
        # Показать список статей для выбора
        articles = await db.get_all_articles()
        if not articles:
            await query.edit_message_text("Нет сохранённых статей для прикрепления.", reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🔙 Назад", callback_data="mat_back")]
            ]))
            return MATERIAL_LINK_OR_FILE
        keyboard = []
        for art in articles:
            aid, title, _, _, _, _, lang, _ = art
            keyboard.append([InlineKeyboardButton(f"{title} ({lang})", callback_data=f"mat_select_article_{aid}")])
        keyboard.append([InlineKeyboardButton("🔙 Назад", callback_data="mat_back")])
        await query.edit_message_text("Выберите статью для прикрепления:", reply_markup=InlineKeyboardMarkup(keyboard))
        context.user_data["material_choice"] = "article"
        return MATERIAL_ARTICLE_SELECT
    return MATERIAL_LINK_OR_FILE

async def material_select_article(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    ...
    await query.edit_message_text("✅ Материал с прикреплённой статьёй добавлен!")
    await admin_materials_menu(update, context)
    return ConversationHandler.END

async def mat_back_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    # Возврат к выбору типа вложения
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("📎 Отправить ссылку", callback_data="mat_link")],
        [InlineKeyboardButton("📤 Загрузить файл", callback_data="mat_file")],
        [InlineKeyboardButton("📄 Прикрепить статью", callback_data="mat_article")],
    ])
    await query.edit_message_text("Вы хотите прикрепить ссылку, файл или статью?", reply_markup=keyboard)
    return MATERIAL_LINK_OR_FILE
...
async def material_receive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message and update.message.text:
        ...
        if not title or not desc:
            await update.message.reply_text("Ошибка. Начните заново.")
            return ConversationHandler.END
        await db.add_material(title, desc, link=link)
        await update.message.reply_text("✅ Материал со ссылкой добавлен!")
        ...
        await update.message.reply_text("Отправьте ссылку или файл.")
        return MATERIAL_LINK_OR_FILE
...
async def delete_material_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not materials:
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("🔙 Назад", callback_data="admin_materials")]
        ])
        await query.edit_message_text("Нет материалов для удаления.", reply_markup=keyboard)
        return
    keyboard = []
    for mat in materials:
        mat_id, title, _, _, _, _, _ = mat
        keyboard.append([InlineKeyboardButton(f"❌ {title}", callback_data=f"delmat_{mat_id}")])
    keyboard.append([InlineKeyboardButton("🔙 Назад", callback_data="admin_materials")])
    await query.edit_message_text("Выберите материал для удаления:", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def confirm_delete_material(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text("Точно удалить материал?", reply_markup=keyboard)
...
def run_bot():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            FILE: [
                MessageHandler(filters.Document.ALL | filters.PHOTO | filters.VIDEO, get_file)
            ],
        },
        fallbacks=[CommandHandler("cancel", cancel)],
    )

    # ConversationHandler для рассылки поста
    ...
    # ConversationHandler для материалов
    mat_conv_handler = ConversationHandler(
        ...
        states={
            ...
            MATERIAL_LINK_OR_FILE: [
                CallbackQueryHandler(material_link_or_file, pattern="^mat_link$"),
                CallbackQueryHandler(material_link_or_file, pattern="^mat_file$"),
                CallbackQueryHandler(material_link_or_file, pattern="^mat_article$"),
                CallbackQueryHandler(mat_back_callback, pattern="^mat_back$"),
                MessageHandler(filters.TEXT & ~filters.COMMAND, material_receive),
                MessageHandler(filters.Document.ALL, material_receive),
            ],
            MATERIAL_ARTICLE_SELECT: [
                CallbackQueryHandler(material_select_article, pattern="^mat_select_article_"),
                CallbackQueryHandler(mat_back_callback, pattern="^mat_back$"),
            ],
        },
    ...
    application.add_handler(CallbackQueryHandler(share_article, pattern="^share_"))  # новый обработчик
    ...
    print("Бот запущен с поддержкой deep linking и материалов...")
