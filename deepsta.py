...
# Состояния
LANG, CONTENT_TYPE, TITLE, FILE = range(4)          # для добавления статьи
BROADCAST_MESSAGE = 0                               # для рассылки поста
FEEDBACK_MESSAGE = 1                                # для обратной связи
EDIT_POST_TEXT = 2                                  # для редактирования
...
class Database:
    ...
    def _init_db(self):
        ...
        # Миграция материалов (article_id)
...
async def delete_messages_between(context, chat_id, from_msg_id, to_msg_id):
    for msg_id in range(from_msg_id + 1, to_msg_id):
        try:
            await context.bot.delete_message(chat_id=chat_id, message_id=msg_id)
        except Exception:
            pass

# ========== ОТПРАВКА В КАНАЛ (только текст) ==========
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Очистка личного чата при каждом новом /start
    if chat_type == "private" and "last_start_msg_id" in context.user_data:
        prev_msg_id = context.user_data["last_start_msg_id"]
        asyncio.create_task(delete_messages_between(context, chat_id, prev_msg_id, current_msg_id))
...
async def view_article_by_id(update: Update, context: ContextTypes.DEFAULT_TYPE, article_id: int):
    ...
    if content_type == "text":
        ...
        try:
            await method(chat_id=update.effective_chat.id, document=file_id,
                        caption=f"{title}\n\n🔗 {link}")
        except Exception as e:
            logger.error(f"Ошибка отправки медиа: {e}")
            await update.message.reply_text("Ошибка отправки медиа.")

    # Отправляем кнопку "Скопировать ссылку" отдельным сообщением
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("🔗 Скопировать ссылку", callback_data=f"copy_link_{article_id}")]
    ])
    await context.bot.send_message(
        chat_id=update.effective_chat.id,
...
async def view_article(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if content_type == "text":
        ...
        try:
            await method(chat_id=query.message.chat_id, document=file_id,
                        caption=f"{title}\n\n🔗 {link}")
        except Exception as e:
            logger.error(f"Ошибка отправки медиа: {e}")
            await query.edit_message_text("Ошибка отправки медиа.")

    # Кнопки навигации + копировать ссылку
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("🔙 К списку", callback_data=f"lang_{lang}"),
    ...
    # Если это не текстовый пост, ссылка уже вставлена в подпись выше,
    # но для единообразия отправляем отдельное сообщение со ссылкой только для текстовых
    if content_type == "text":
        await context.bot.send_message(
            chat_id=query.message.chat_id,
            text=f"🔗 Ссылка на статью:\n{link}"
        )
...
async def materials_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for mat in materials:
        ...
        keyboard.append([InlineKeyboardButton(f"{prefix} {title}", callback_data=f"material_{mat_id}")])
...
# ========== ОБРАТНАЯ СВЯЗЬ (возвращает поле ввода) ==========
async def feedback_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.callback_query:
        ...
        # Удаляем предыдущее меню, чтобы не висело два сообщения
        try:
...
async def broadcast_send(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.text:
        ...
        asyncio.create_task(send_to_channel(context, text="📢 Новый пост в боте!"))
    elif update.message.photo:
        file_id = update.message.photo[-1].file_id
        caption = update.message.caption or ""
        asyncio.create_task(broadcast_post(context, "photo", file_id=file_id, caption=caption))
        asyncio.create_task(send_to_channel(context, text="📢 Новый пост с фото в боте!"))
    elif update.message.video:
        file_id = update.message.video.file_id
        caption = update.message.caption or ""
        asyncio.create_task(broadcast_post(context, "video", file_id=file_id, caption=caption))
        asyncio.create_task(send_to_channel(context, text="📢 Новый пост с видео в боте!"))
    elif update.message.document:
        file_id = update.message.document.file_id
        caption = update.message.caption or ""
        asyncio.create_task(broadcast_post(context, "document", file_id=file_id, caption=caption))
        asyncio.create_task(send_to_channel(context, text="📢 Новый пост с документом в боте!"))
    else:
        await update.message.reply_text("Неизвестный тип сообщения.")
        return BROADCAST_MESSAGE

    await update.message.reply_text("✅ Пост разослан и уведомление отправлено в канал!")
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("➕ Добавить статью", callback_data="add_article")],
        [InlineKeyboardButton("📢 Создать пост", callback_data="broadcast")],
        [InlineKeyboardButton("📝 Управление постами", callback_data="manage_posts")],
...
async def get_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if content_type == "pdf":
        ...
        try:
            ...
            asyncio.create_task(send_to_channel(context, text=f"📄 <b>Новая статья!</b>\n{title}\nЯзык: {lang}"))
        ...
        try:
            ...
            asyncio.create_task(broadcast_new_article(context, title, lang, article_id))
            asyncio.create_task(send_to_channel(context, text=f"🖼️ <b>Новая статья!</b>\n{title}\nЯзык: {lang}"))
...
async def material_link_or_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if choice == "link":
        ...
        await query.edit_message_text("Выберите статью:", reply_markup=InlineKeyboardMarkup(keyboard))
...
def run_bot():
    ...
    # Копировать ссылку
    application.add_handler(CallbackQueryHandler(copy_link_handler, pattern="^copy_link_"))
    # Материалы
    ...
    print("Бот запущен с кнопкой «Скопировать ссылку»...")
