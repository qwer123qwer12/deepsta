...
DATA_DIR = os.environ.get("DATA_DIR", ".")          # Для Render Disk (напр. /opt/render/project/src/data)

ARTICLES_DIR = os.path.join(DATA_DIR, "articles_pdf")
...
async def broadcast_send(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.text:
        ...
        # Посты с медиа пересылаем как есть (пользователь ожидает увидеть пост, а не просто текст)
        asyncio.create_task(send_to_channel(context, text=caption))  # для канала лучше текст-уведомление? Нет, для постов логично кидать сам пост.
        # Но пользователь не уточнял, оставлю как было: для постов в канал идёт сам пост (медиа/текст)
...
# --- Добавление статьи (в канал – ТОЛЬКО УВЕДОМЛЕНИЕ) ---
...
async def get_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if content_type == "pdf":
        ...
        try:
            ...
            # В канал отправляем только уведомление
            link = make_article_link(article_id)
...
def run_bot():
    ...
    print("Бот запущен: уведомления в канал, ссылки, возврат из материалов, поле ввода...")
